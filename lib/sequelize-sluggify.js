...
class SequelizeSluggify {
    sluggifyModel(Model, options) {
        ...
        if(typeof options.reserved === "undefined") {
            options.reserved = [];
        }
        ...
        var isAvailable = (slug) => {
            if(options.reserved.indexOf(slug) != -1) {
        ...
        var addNumberSuffix = (slug, count) => {
            ...
            .then((available) => {
                if(available) {
                    return Promise.resolve(suffixedSlug);
                }
        ...
        var handleSluggify = (instance, sequelizeOptions) => {
            var valuesChanged = options.source.some((field) => {
